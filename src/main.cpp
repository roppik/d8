...
#if defined(NDEBUG)
# error "D8coin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x5dda37b5afa47b243229a55bff80f7cfef8dacd2dced312be5b202cba5c04c5f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // D8coin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "D8coin Signed Message:\n";
...
{
    // D8coin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // D8coin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // D8coin: 1 days
static const int64 nTargetSpacing = 2.5 * 60; // D8coin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // D8coin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // D8coin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x897acfb1b941497e938c4820be98b3c6d9f9aeaca774c3aa2a131c7641dfe482");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Roppik Slovakia 2019";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 10000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04de37295468b4ce0b7a3cf967ad4b5aae35e8fdb6ad98579945e5dc7a08bd9aac4b2177f78884915029832c029075e8bdb6e3a7001af2ba68a79fafc72594e3c2") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1550775970;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085798142;

        if (fTestNet)
        {
            block.nTime    = 1550775747;
            block.nNonce   = 388749967;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xe9330a90e4d0bd02e8962b0ef67424ee67a1dc4e76dccd186f9b4afbc6006040"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf5, 0xc4, 0xb6, 0xd1 }; // D8coin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("D8coinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("D8coinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static D8coinMiner(CWallet *pwallet)
{
    printf("D8coinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("d8coin-miner");
    ...
    try { loop {
        ...
        printf("Running D8coinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&D8coinMiner, pwallet));
}
